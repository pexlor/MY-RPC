使用的技术：
Liunx + protobuf + tinyxml + zookeeper +c++
...
> 消息发送错误（解决），接受到的meeage传递的给工作线程时，由于传递的是一个（char*）指针，持有该指针的string对象在传递完后便销毁，故导致发送错误。解决方案为使用unique_ptr指针。
