...
PATH_TCP = $(PATH_ROCKET)/net/PxLib
PATH_CODER = $(PATH_ROCKET)/net/Rpc/coder
...
ALL_TESTS : $(PATH_BIN)/test_log  $(PATH_BIN)/test_rpc_client $(PATH_BIN)/test_rpc_server

TEST_CASE_OUT := $(PATH_BIN)/test_log  $(PATH_BIN)/test_rpc_client $(PATH_BIN)/test_rpc_server
...
$(PATH_BIN)/test_rpc_client: $(LIB_OUT)
	$(CXX) $(CXXFLAGS) $(PATH_TESTCASES)/test_rpc_client.cc $(PATH_TESTCASES)/order.pb.cc -o $@ $(LIB_OUT) $(LIBS) -ldl -pthread -lprotobuf

$(PATH_BIN)/test_rpc_server: $(LIB_OUT)
	$(CXX) $(CXXFLAGS) $(PATH_TESTCASES)/test_rpc_server.cc $(PATH_TESTCASES)/order.pb.cc -o $@ $(LIB_OUT) $(LIBS) -ldl -pthread -lprotobuf
